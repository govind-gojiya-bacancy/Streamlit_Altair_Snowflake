...
# Load secrets
snowflake_secrets = st.secrets["expense_tracker"]
...
# CRUD operations with Snowflake Connector
def create_expense(conn, item_name, category, amount, payment_mode, purchase_date, description):
    query = f"""
    INSERT INTO EXPENSES (ITEM_NAME, CATEGORY, AMOUNT, PAYMENT_MODE, PURCHASE_DATE, DESCRIPTION)
    VALUES (%s, %s, %s, %s, %s, %s)
    """
    with conn.cursor() as cur:
        cur.execute(query, (item_name, category, amount, payment_mode, purchase_date, description))
        conn.commit()

def read_expenses(conn):
    query = "SELECT * FROM EXPENSES"
    return pd.read_sql(query, conn)

def update_expense(conn, expense_id, item_name, category, amount, payment_mode, purchase_date, description):
    query = f"""
    UPDATE EXPENSES
    SET ITEM_NAME = %s, CATEGORY = %s, AMOUNT = %s, PAYMENT_MODE = %s, PURCHASE_DATE = %s, DESCRIPTION = %s
    WHERE ID = %s
    """
    with conn.cursor() as cur:
        cur.execute(query, (item_name, category, amount, payment_mode, purchase_date, description, expense_id))
        conn.commit()
...
# Streamlit UI
st.title('Expense Tracker using SnowFlake connector')
...
if submit_button:
    create_expense(conn, item_name, category, amount, payment_mode, purchase_date, description)
    st.success('Expense added successfully!')

# Display expenses
expenses_df = read_expenses(conn)
st.write('### Expenses')
...
if not expenses_df.empty:
    ...
    if update_button:
        update_expense(conn, expense_id, item_name, category, amount, payment_mode, purchase_date, description)
